/-
  Formalisation/Hyperlocal/Targets/XiPacket/XiRow0Bridge_AtOrderCoords01FromSigmaAndRow012TrueAnalyticStrip.lean

  Strip-specialised coords01-at-order source from:

    * XiAtOrderSigmaProvider
    * XiRow012UpstreamTrueAnalyticStrip
    * strip-native σ3 ≠ 0

  This is the strip companion of
    XiRow0Bridge_AtOrderCoords01FromSigmaAndRow012TrueAnalytic.lean
  and avoids the global `XiSigma3Nonzero` / `A0Nonzero` route.
-/
...
theorem xiAtOrderCoords01Out_of_sigmaAndRow012TrueAnalytic_strip
    [XiRow012UpstreamTrueAnalyticStrip]
    (m : ℕ) (s : _root_.Hyperlocal.OffSeedStrip Xi)
    (Hσ : XiAtOrderSigmaOut m (s : OffSeed Xi)) :
    XiAtOrderCoords01Out m (s : OffSeed Xi) := by
  classical
  let s0 : OffSeed Xi := (s : OffSeed Xi)
  let hσ3 : (JetQuotOp.σ3 s0 : ℂ) ≠ 0 :=
    sigma3_ne_zero_of_strip (s := s)

  have Hw0 :
      (w0At m s0) 0 = 0 ∧ (w0At m s0) 1 = 0 ∧ (w0At m s0) 2 = 0 :=
    coords012_eq_zero_of_sigma_and_row012
      (s := s0) (z := s0.ρ) (w := w0At m s0)
      (hSigma := Hσ.hw0AtSigma)
      (H := (XiRow012UpstreamTrueAnalyticStrip.row012_out_strip (m := m) (s := s)).hw0At)
      (hσ3 := hσ3)

  have Hwp2 :
      (wp2At m s0) 0 = 0 ∧ (wp2At m s0) 1 = 0 ∧ (wp2At m s0) 2 = 0 :=
    coords012_eq_zero_of_sigma_and_row012
      (s := s0) (z := (starRingEnd ℂ) s0.ρ) (w := wp2At m s0)
      (hSigma := Hσ.hwp2AtSigma)
      (H := (XiRow012UpstreamTrueAnalyticStrip.row012_out_strip (m := m) (s := s)).hwp2At)
      (hσ3 := hσ3)

  have Hwp3 :
      (wp3At m s0) 0 = 0 ∧ (wp3At m s0) 1 = 0 ∧ (wp3At m s0) 2 = 0 :=
    coords012_eq_zero_of_sigma_and_row012
      (s := s0) (z := (1 - (starRingEnd ℂ) s0.ρ)) (w := wp3At m s0)
      (hSigma := Hσ.hwp3AtSigma)
      (H := (XiRow012UpstreamTrueAnalyticStrip.row012_out_strip (m := m) (s := s)).hwp3At)
      (hσ3 := hσ3)
...
theorem xiAtOrderCoords01Out_fromSigmaAndRow012TrueAnalytic_strip
    [XiAtOrderSigmaProvider]
    [XiRow012UpstreamTrueAnalyticStrip]
    (m : ℕ) (s : _root_.Hyperlocal.OffSeedStrip Xi) :
    XiAtOrderCoords01Out m (s : OffSeed Xi) := by
  classical
  have Hσ : XiAtOrderSigmaOut m (s : OffSeed Xi) :=
    xiAtOrderSigmaOut_provided (m := m) (s := (s : OffSeed Xi))
  exact xiAtOrderCoords01Out_of_sigmaAndRow012TrueAnalytic_strip
    (m := m) (s := s) Hσ
