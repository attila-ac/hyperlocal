/-
  Hyperlocal/Targets/XiPacket/XiRow0Bridge_JetWindowEqFromRouteA_Core.lean

  Core API for Route-A jet window equalities.

  IMPORTANT:
  * This file contains NO fallback axiom imports.
  * It only develops projection lemmas + window=jet3 theorems under
    `[RouteAJetCoordProvider]`.

  Rationale:
  This module exists to avoid import cycles when other theorem modules
  (e.g. JetLeibnizAtFromRouteA) need the window=jet3 facts but must not
  pull in the fallback axiom provider.
-/
