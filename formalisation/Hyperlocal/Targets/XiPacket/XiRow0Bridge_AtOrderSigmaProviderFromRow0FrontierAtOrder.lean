/-
  ...
  Then rewrite row-0 ToeplitzL into `row0Sigma` using the cycle-safe
  algebra bridge `row0Sigma_eq_zero_of_toeplitz_row0_eq_zero`.
...
theorem xiAtOrderSigmaOut_fromRow0FrontierAtOrder
  ...
  · -- w0At
    ...
    exact row0Sigma_eq_zero_of_toeplitz_row0_eq_zero (s := s) (w := w0At m s) ht
  · -- wp2At
    have ht : (toeplitzL 2 (JetQuotOp.aRk1 s) (wp2At m s)) (0 : Fin 3) = 0 :=
      xiJetQuot_row0_wp2At (m := m) (s := s)
    exact row0Sigma_eq_zero_of_toeplitz_row0_eq_zero (s := s) (w := wp2At m s) ht
  · -- wp3At
    have ht : (toeplitzL 2 (JetQuotOp.aRk1 s) (wp3At m s)) (0 : Fin 3) = 0 :=
      xiJetQuot_row0_wp3At (m := m) (s := s)
    exact row0Sigma_eq_zero_of_toeplitz_row0_eq_zero (s := s) (w := wp3At m s) ht
