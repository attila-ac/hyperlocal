...
theorem xiRow012ConvolutionAtRevAtOrderOut_fromAnalytic_strip
  ...
  · rcases JetQuotOp.xiRouteA_jetPkg_w0At (m := m) (s := s0) with
    ...
    have H0 : Row0ConvolutionAtRev s0 (s0.ρ) (w0At m s0) := ⟨G, hfac, hjet, h3⟩
  ...
  · rcases JetQuotOp.xiRouteA_jetPkg_wp2At (m := m) (s := s0) with
    ...
    have H0 : Row0ConvolutionAtRev s0 ((starRingEnd ℂ) s0.ρ) (wp2At m s0) := ⟨G, hfac, hjet, h3⟩
    exact row012ConvolutionAtRev_of_row0ConvolutionAtRev_and_coords
  ...
  · rcases JetQuotOp.xiRouteA_jetPkg_wp3At (m := m) (s := s0) with
    ...
    have H0 : Row0ConvolutionAtRev s0 (1 - (starRingEnd ℂ) s0.ρ) (wp3At m s0) := ⟨G, hfac, hjet, h3⟩
    exact row012ConvolutionAtRev_of_row0ConvolutionAtRev_and_coords
      (s := s0) (z := (1 - (starRingEnd ℂ) s0.ρ)) (w := wp3At m s0) H0 Hcoords.hwp3At0 Hcoords.hwp3At1
